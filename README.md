...
| `/ferrite worldgen map-memo on\|off\|status` | Each noise router node is mapped once per `NoiseChunk` instead of at every reference, giving the same cache objects in the same order. Makes `NoiseChunk` construction (per chunk and per terrain height query) about half as costly with big datapack routers such as Terralith's. One `NoiseChunk` in 64 is checked against the full mapping. Session only; `-Dferrite.worldgen.mapmemo=false`. | **on** |
| `/ferrite worldgen noise-math on\|off\|verify\|status` | Noise sampling with the same arithmetic in fewer steps: an octave coordinate under 2^23 skips the wrap arithmetic that returns it unchanged, and corner sampling reads an int permutation and a flat table of the same gradients. Same bits (Java floating point is strict); the first 200,000 samples are checked against the game's code. About 9% off the noise stage of a chunk. Session only; `-Dferrite.worldgen.noisemath=false`. | **on** |
| `/ferrite worldgen surface-prune on\|off\|status` | Surface rule sequences are built without the biome tests that cannot pass in the chunk (the game already makes those constant false or true per chunk, but still walked them at every block). The same blocks; one sequence in 64 is checked result by result. About 13% off the surface step, which runs one chunk at a time. Session only; `-Dferrite.worldgen.surfaceprune=false`. | **on** |
| `/ferrite worldgen biome-fold on\|off\|status` | Climate Rivers' surface biome test becomes the game's constant condition in chunks where its answer cannot vary, as the game's own biome test does, so surface rule pruning drops it. The same blocks; one fold in 64 is checked at every block. Up to 10% off the surface step (a few percent in live generation). Only with Climate Rivers. Session only; `-Dferrite.worldgen.biomefold=false`. | **on** |
| `/ferrite worldgen lazy-interp on\|off\|status` | Interpolated noise values are worked out when read instead of being stepped for every interpolator at every block; the same lerp of the same inputs. One `NoiseChunk` in 64 is checked read by read. About 4% off the noise stage of a chunk. Session only; `-Dferrite.worldgen.lazyinterp=false`. | **on** |
...
| `/ferrite compat lithostitched\|spawners\|roguelike\|all on\|off`, `/ferrite compat status` | Chunk-wait guards. Three checks that asked only whether a chunk was loaded (scheduled, maybe not generated yet) and then read it, making the server thread generate it on the spot, now wait until it is generated: Lithostitched's structure attribute check, the cat spawner, and Roguelike Dungeons' room check. Only timing changes. On the players bench they cut freezes from 31.5 s to 3.4 s in 3 minutes of mixed exploring. Session only; `-Dferrite.compat.<name>=false`. | **on** |
...
2. **Let C2ME handle stragglers, even on 2 cores.** For players wandering past the pre-generated border, C2ME generates the remaining chunks. The CI players bench models a 2-core, 4-thread laptop CPU with a walker, a rider and an elytra exploring at once. There, C2ME sent players about 3 times as many chunks as the same server without it, in both of two repeats. It spent a third of the worker CPU per chunk sent. With C2ME loaded, Ferrite's worldgen hooks on code C2ME rewrites stand down; the rest of Ferrite, including the chunk-wait guards, still applies. (With C2ME loaded, Ferrite's `/ferrite density validate` diagnostic reports false failures. Gameplay is unaffected.) Elytra and fast boats still outrun generation on a CPU this size. Over a pregenerated corridor, the bench's elytra received 51 chunks/s against 2-4 over fresh terrain, so pregenerate the area flyers use.
