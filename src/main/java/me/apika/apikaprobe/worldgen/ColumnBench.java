 ...
 * builds a whole NoiseChunk for one column) with DensityWrapIndex on and
 * off, alternating the order each round, and checks the heights agree.
 * Runs on the calling thread; for the CI worldgen bench.
 */
public final class ColumnBench {
	...
	public static String run(ServerLevel level, int queries, int rounds) {
		...
		boolean saved = DensityWrapIndex.ENABLED;
		long[][] nanos = new long[2][rounds];
		int[][] heights = new int[2][queries];
		try {
			// Warm up both paths.
			for (int arm = 0; arm < 2; arm++) {
				DensityWrapIndex.ENABLED = arm == 0;
			...
			for (int round = 0; round < rounds; round++) {
				for (int step = 0; step < 2; step++) {
					int arm = (round + step) % 2;   // 0 = index on, 1 = off
					DensityWrapIndex.ENABLED = arm == 0;
			...
			DensityWrapIndex.ENABLED = saved;
		}
		int differing = 0;
		for (int i = 0; i < queries; i++) {
		...
		return String.format("[column-bench] queries=%d rounds=%d us/query median: index on %.1f, off %.1f (%+.1f%%); heights differing %d;%s",
